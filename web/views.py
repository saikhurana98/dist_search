...
class Movie:
    def __init__(self, title=None, description=None, img_url=None, genre=None, runtime=None, imdb_url=None):
...
def getMovies(id_list=[], search_string=''):
    ...
    for movie_id in id_list:
        try:
            ...
            c.execute(""" SELECT title, description, img_url, genre, runtime, imdb_url FROM Movie WHERE id = :id """, {
                      'id': movie_id})
            movie = c.fetchone()
            m = Movie(title=movie[0], description=movie[1], img_url=movie[2],
                      genre='/ '.join(eval(movie[3])), runtime=movie[4], imdb_url=movie[5])
